...
const NoMembersView: React.FC<Props> = (props) => {
  // Theme
  const theme = useContext(ThemeContext);
  const themeType = useAgile(core.ui.THEME_TYPE);

  // Images
  const [noMembersImagePath_Dark] = useState(
    require("../../../../../assets/images/News/news_dark.png")
  );
  const [noMembersImagePath_Light] = useState(
    ...
    <CenterContainer>
      ...
      >
        <CImage
          ...
          source={
            themeType === "dark"
              ? noMembersImagePath_Dark
              : noMembersImagePath_Light
          }
          resizeMode={"contain"}
        />
        <CText color={theme.colors.on_surface}>
          {strings().noMemberFoundInfoText}
        </CText>
      </Animated.View>
    </CenterContainer>
  );
